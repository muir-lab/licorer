...
This vignette will show you how to take subsets of LI-COR data or combine multiple licor objects into a single object using the **licorer** package.
...
### Combining multiple licor objects

The `combine_licor()` method takes a list of licor objects and fuses them into a singular licor object. It merges licors by combining their variables, creating `NA` values if a variable is not present in one of the licor values. The data will be accompanied by a file index associated with the header that indicates which file the data and header values are associated with. It is shown in the first column `file_index`

```{r}
test_data2 <- licor(system.file("extdata", "2019-05-06-0740_trillium_ovatum", 
                               package = "licorer", mustWork = TRUE))

multilicor <- combine_licor(list(test_data, test_data2))
```

```{r, eval = TRUE, echo = FALSE}
scroll_box(knitr::kable(head(multilicor), caption = "Example"),
            height = "250px", width = "700px", 
            box_css = "border: 1px solid #ddd; padding: 5px; ", 
            extra_css = NULL, fixed_thead = TRUE)
```
