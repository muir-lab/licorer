...
Most of the units in the data have units which are automatically assigned to them
when the file is loaded. To make this easier to parse, rows fall into a few different categories which are stored as attributes to the row names. These are covered in greater detail in the vignette on data operations, but are given by the "subset_options" method.
