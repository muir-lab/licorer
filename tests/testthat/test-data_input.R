...
test_that("read_li6800_raw creates a licor object", {
  ...
  x <- read_li6800(system.file("extdata", "2019-05-06-0740_trillium_ovatum",
                                   package = "licorer", mustWork = TRUE))
  expect_equal(class(x), c("licor", "tbl_df", "tbl", "data.frame"))
  #Test reading in with a file path
  #x <- read_li6800_raw(rprojroot::find_root_file("2019-05-06-0740_trillium_ovatum"))
  #expect_equal(class(x), c("licor", "tbl_df", "tbl", "data.frame"))
})

test_that("read_li6400_raw creates a licor object", {
  #Test reading in with an internal file
  x <- read_li6400_raw(system.file("extdata", "chris0701_ACi_chi2",
                                   package = "licorer", mustWork = TRUE))
  expect_equal(class(x), c("licor", "tbl_df", "tbl", "data.frame"))

  x <- read_li6400(system.file("extdata", "chris0701_ACi_chi2",
                                   package = "licorer", mustWork = TRUE))
  expect_equal(class(x), c("licor", "tbl_df", "tbl", "data.frame"))
})
